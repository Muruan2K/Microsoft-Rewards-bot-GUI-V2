...
class Telegram(ft.UserControl):
    ...
    def set_initial_values(self):
        ...
        if self.page.client_storage.contains_key("MRFarmer.telegram_proxy_switch"):
            self.proxy_field.disabled = not self.page.client_storage.get("MRFarmer.telegram_proxy_switch")
        else:
            self.page.client_storage.set("MRFarmer.telegram_proxy_switch", False)
            self.proxy_field.disabled = True
        self.telegram_proxy_switch.value = self.page.client_storage.get("MRFarmer.telegram_proxy_switch")
        self.proxy_field.value = self.page.client_storage.get("MRFarmer.telegram_proxy")
    ...
    def save(self, e):
        if self.are_telegram_fields_filled():
            ...
            self.page.client_storage.set("MRFarmer.telegram_proxy_switch", self.telegram_proxy_switch.value)
            self.page.client_storage.set("MRFarmer.telegram_proxy", self.proxy_field.value)
    ...
    def send_test_message(self, e):
        ...
        try:
            ...
            data = {"chat_id": self.chat_id_field.value, "text": self.test_message_field.value}
            if self.proxy_field.value != "" and self.is_proxy_working(self.proxy_field.value):
                requests.post(url, json=data, proxies={"https": f"http://{self.proxy_field.value}"})
            else:
    ...
    def is_proxy_working(self, proxy):
        try:
            requests.get("https://google.com", proxies={"https": f"http://{proxy}"}, timeout=5)
            return True
